...
bool Simulation<ndim>::ReadSerenFormSnapshotFile(string filename)
{
  ...
  for (j=0; j<ndata; j++) {
    ...
    // outflowFlag (only for analysing)
    //---------------------------------------------------------------------------------------------
    else if (data_id[j] == "outflowFlag") {
      for (i=0; i<hydro->Nhydro; i++) {
        Particle<ndim>& part = hydro->GetParticlePointer(i);
        infile >> part.outflowFlag;
				part.outflowFlag /= simunits.t.outscale;
      }
    }
    ...
    //-------------------------------------------------------------------------
      ...
      for (i=0; i<hydro->Nhydro; i++) {
        Particle<ndim>& part = hydro->GetParticlePointer(i);
        infile >> part.gpot;
				part.gpot /= simunits.E.outscale*part.m;
      }
    }
    ...
    //---------------------------------------------------------------------------------------------
      ...
      if (nbody->Nstar > 0) {
        for (i=0; i<nbody->Nstar; i++) {
          ...
          //for (ii=0; ii<sink_data_length; ii++) cout << "data: " << ii << " :     "  << sdata[ii] << endl;
          ...
          sinks->sink[i].radius = sdata[3+2*ndim];
					sinks->sink[i].ketot = sdata[4+2*ndim]/simunits.E.outscale;
					sinks->sink[i].gpetot = sdata[5+2*ndim]/simunits.E.outscale;
					sinks->sink[i].rotketot = sdata[6+2*ndim]/simunits.E.outscale;
					sinks->sink[i].utot = sdata[7+2*ndim]/simunits.E.outscale;

          // Extra stuff for episodic sink accretion
          for (k=0; k<ndim; k++) sinks->sink[i].L_star[k] = sdata[9 + 2*ndim + k]/simunits.angmom.outscale;
          sinks->sink[i].acc_flag 			= (int) sdata[12 + 2*ndim];
          sinks->sink[i].DT_MRI   			= sdata[12 + 2*ndim + 1]/simunits.t.outscale;
          sinks->sink[i].dmdt_MRI_0 		= sdata[12 + 2*ndim + 2]/simunits.dmdt.outscale;
          sinks->sink[i].t_old   				= sdata[12 + 2*ndim + 3]/simunits.t.outscale;
          sinks->sink[i].m_old    			= sdata[12 + 2*ndim + 4]/simunits.m.outscale;
          sinks->sink[i].M_star   			= sdata[12 + 2*ndim + 5]/simunits.m.outscale;
          sinks->sink[i].t0       			= sdata[12 + 2*ndim + 6]/simunits.t.outscale;
      	  sinks->sink[i].M_star_old			= sdata[12 + 2*ndim + 7]/simunits.m.outscale;
      	  sinks->sink[i].dm_store				= sdata[12 + 2*ndim + 8]/simunits.m.outscale;
          for (k=0; k<ndim; k++) sinks->sink[i].angmom[k] = sdata[12 + 2*ndim + 9 + k]/simunits.angmom.outscale;
      	  sinks->sink[i].dmdt_star			= sdata[13 + 3*ndim + 9]/simunits.dmdt.outscale;
      	  sinks->sink[i].stellarRadius	= sdata[13 + 3*ndim + 10]/simunits.r.outscale;
      	  sinks->sink[i].stellarPI			= sdata[13 + 3*ndim + 11];
      	  sinks->sink[i].stellarMd			= sdata[13 + 3*ndim + 12]/simunits.m.outscale;
      	  sinks->sink[i].stellarStage		= (int) sdata[13 + 3*ndim + 13];
      	  sinks->sink[i].luminosity			= sdata[13 + 3*ndim + 14]/simunits.L.outscale;
        }
      }
    }
 ...
 (string filename)                               ///< [in] Filename to write new snapshot to
{
  ...
  if (Nlivehydro > 0) {
    ...
    data_id[ndata] = "outflowFlag";
    typedata[ndata][0] = 1;              typedata[ndata][1] = 1;
    typedata[ndata][2] = Nlivehydro;  	 typedata[ndata][3] = 4;
    typedata[ndata][4] = 3;              ndata++;

    data_id[ndata] = "gpot";
    typedata[ndata][0] = 1;              typedata[ndata][1] = 1;
    typedata[ndata][2] = Nlivehydro;  	 typedata[ndata][3] = 4;
    typedata[ndata][4] = 10;             ndata++;
  ...
  if (Nlivehydro > 0) {
    ...
    //-------------------------------------------------------------------------
      ...
      if (!part.flags.is_dead()) {
				outfile << part.outflowFlag*simunits.t.outscale << endl;
      }
		}
    ...
    //-------------------------------------------------------------------------
		for (i=0; i<hydro->Nhydro; i++){
			Particle<ndim>& part = hydro->GetParticlePointer(i);
      if (!part.flags.is_dead()) {
				outfile << part.gpot*part.m*simunits.E.outscale << endl;
      }
		}
  ...
  if (nbody->Nstar > 0) {
    ...
    for (i=0; i<nbody->Nstar; i++) {
      ...
      sdata[4+2*ndim] = sinks->sink[i].ketot*simunits.E.outscale;
      sdata[5+2*ndim] = sinks->sink[i].gpetot*simunits.E.outscale;
      sdata[6+2*ndim] = sinks->sink[i].rotketot*simunits.E.outscale;
      sdata[7+2*ndim] = sinks->sink[i].utot*simunits.E.outscale;

      // Extra quantities for episodic sink accretion
      for (k=0; k<ndim; k++) sdata[9 + 2*ndim + k] = sinks->sink[i].L_star[k]*simunits.angmom.outscale;
      sdata[12 + 2*ndim]     = (FLOAT) sinks->sink[i].acc_flag + (FLOAT) 0.01;
      sdata[12 + 2*ndim + 1] = sinks->sink[i].DT_MRI*simunits.t.outscale;
      sdata[12 + 2*ndim + 2] = sinks->sink[i].dmdt_MRI_0*simunits.dmdt.outscale;
      sdata[12 + 2*ndim + 3] = sinks->sink[i].t_old*simunits.t.outscale;
      sdata[12 + 2*ndim + 4] = sinks->sink[i].m_old*simunits.m.outscale;
      sdata[12 + 2*ndim + 5] = sinks->sink[i].M_star*simunits.m.outscale;
      sdata[12 + 2*ndim + 6] = sinks->sink[i].t0*simunits.t.outscale;
      sdata[12 + 2*ndim + 7] = sinks->sink[i].M_star_old*simunits.m.outscale;
      sdata[12 + 2*ndim + 8] = sinks->sink[i].dm_store*simunits.m.outscale;
      for (k=0; k<ndim; k++) sdata[12 + 2*ndim + 9 + k] = sinks->sink[i].angmom[k]*simunits.angmom.outscale;
      sdata[13 + 3*ndim + 9] = sinks->sink[i].dmdt_star*simunits.dmdt.outscale;
      sdata[13 + 3*ndim + 10] = sinks->sink[i].stellarRadius*simunits.r.outscale;
      sdata[13 + 3*ndim + 11] = sinks->sink[i].stellarPI;
      sdata[13 + 3*ndim + 12] = sinks->sink[i].stellarMd*simunits.m.outscale;
      sdata[13 + 3*ndim + 13] = sinks->sink[i].stellarStage;
      sdata[13 + 3*ndim + 14] = sinks->sink[i].luminosity*simunits.L.outscale;
