  ...
  NeighbourList<typename GradhSphBase<ndim>::HydroNeib>& neibpart) ///< [in] List of neighbours
{
  ...
  if (tdavisc == mm97) {
    parti.tdav = (FLOAT) 0.1*parti.sound*(alpha_visc_min - parti.alpha)*invh_i +
 ...
 NeighbourList<typename GradhSphBase<ndim>::HydroNeib>& neibpart) ///< [in] List of neighbours
{
  ...
  if (tdavisc == mm97) {
	...
	if (parti.dudt > pow(10,20)) cout << parti.dudt<< " " << parti.u << " " <<   " dudt " <<  eos->Pressure(parti) << " " << parti.div_v << " " << invrho_i << " " << parti.invomega << endl;
  return;
}
...
void GradhSph<ndim, kernelclass>::FinishReturnExport () {
  if (tdavisc == mm97) {
    for (int i=0; i<Nhydro; i++) {
      ...
      part.tdav = (FLOAT) 0.1*part.sound*(alpha_visc_min - part.alpha)/part.h +
