...
void UniformIc<ndim>::Generate(void)
{
  ...
  //-----------------------------------------------------------------------------------------------
    ...
    press  /= simunits.press.outscale;
  	...
  	Nshell = int (M_shell / (mcloud/Npart));
  	r_shell = new FLOAT[ndim*Nshell];
  	cout << "Ncore: " << Npart << "   Nshell:  " << Nshell << "    Mshell: " << M_shell << endl;
    ...
    shared(gammaone,mcloud,Npart,press,r,rhofluid,volume, temp0, M_shell, T_ambient, R_ambient, rho_ambient, r_shell) private(i,k)
    for (i=0; i<hydro->Nhydro; i++) {
      Particle<ndim>& part = hydro->GetParticlePointer(i);
      if(i<Npart){
      	for (k=0; k<ndim; k++) {
      ...
      part.u = temp0/part.mu_bar/(part.gamma- (FLOAT) 1.0);
      part.temp_ambient = temp0;
      part.iorig = i;
    }
    if (ndim == 3) {
	#pragma omp parallel for default(none)\
	shared(r_perp, pc, Npart, omega) private(i,k)
      for (i=0; i<Npart; i++) {
        Particle<ndim>& part = hydro->GetParticlePointer(i);
        part.iorig = i;
