...
void IsothermalSphereIc<ndim>::Generate(void)
{
  ...
  //-----------------------------------------------------------------------------------------------
    ...
    // Record particle positions and initialise all other variables
    #pragma omp parallel for default(none) shared(gammaone,mcloud,Npart,press,r,rhofluid,volume,temp0,mu_bar) private(i,k)
    for (i=0; i<Npart; i++) {
      Particle<ndim>& part = hydro->GetParticlePointer(i);
      for (k=0; k<ndim; k++) {
        part.r[k] = r[ndim*i + k];
      ...
      part.u = temp0/(part.gamma - (FLOAT) 1.0)/part.mu_bar;
      part.m = mcloud / (FLOAT) Npart;
      part.h = hydro->h_fac*pow(part.m/rhofluid, invndim);	
    }
    for (i=0; i<Nshell; i++) {
