  ...
  DOUBLE *vfield)                      ///< [out] Array containing velocity field grid
{
  ...
  if (ndim == 3) {
    ...
    kminMode = simparams->intparams["kMinMode"];
    ...
    cout << "kmin : " << kmin << "    kmax : " << kmax << "    krange : "
         << krange << "    gridsize : " << gridsize << "    kminMode :" << kminMode << endl;
    if (krange != gridsize) {
			kmin -= 2;
		}
    if (krange != gridsize) {
...
//=================================================================================================
//  Ic::SphericalShell
/// Add random Spherical shell of particles
//=================================================================================================
template <int ndim>
void Ic<ndim>::SphericalShell
 (const int Npart,                     ///< [in] No. of particles in sphere
  const FLOAT rcentre[ndim],           ///< [in] Position of sphere centre
  const FLOAT r_min,                  ///< [in] Radius of sphere
  const FLOAT r_max,
  FLOAT *r)                            ///< [out] Positions of particles in sphere
{
  int i,k;                             // Particle and dimension counters
  ...
  for (i=0; i<Npart; i++) {
    ...
    // Continously loop until random particle lies inside sphere
    do {
    	for (k=0; k<ndim; k++)rpos[k] = (FLOAT) r_max - 2* sim->randnumb->floatrand()*r_max;
		rad = sqrt(DotProduct(rpos,rpos,ndim));
    } while (rad < r_min or rad > r_max);
