...
//=================================================================================================
template <int ndim>
void MeshlessFV<ndim>::UpdateArrayVariables(MeshlessFVParticle<ndim> &part, FLOAT Qcons[nvar], FLOAT timestep)
{
  FLOAT ekin0 = (FLOAT) 0.0;
  FLOAT m0 = part.m;
  //if (types[part.ptype].hydro_forces) {
    for (int k=0; k<ndim; k++) ekin0 += Qcons[k]/part.m;
  //}
  ekin0 *= (FLOAT) 0.5 * part.m;
  ...
  if (types[part.ptype].hydro_forces) {
    ...
    if (part.flags.check(end_timestep)) {
    	part.dudt 	= ((Qcons[ietot] - (FLOAT) 0.5*part.m*ekin)/part.m - (part.Qcons0[ietot] - ekin0)/m0)/(timestep + small_number);
    	cout << part.rho  << " " << part.dudt << endl;
    }
    part.u 		= eos->SpecificInternalEnergy(part);
    part.press 	= eos->Pressure(part);
